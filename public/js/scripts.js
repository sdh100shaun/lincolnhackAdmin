...
function menuFunctionality() {
  ...
  var body = $("html, body"); // On click of the nav button show the navigation.
  ...
  $('[data-nav-ref]').click(function (e) {
    e.preventDefault(); // Hide the navbar.

    $(mainNav).removeClass('showing'); // Get the tag.

    var ref = $(this).attr('href'); // Animate the scrollation. SCROLLATION?

    if (ref.length > 0) {
      body.stop().animate({
        scrollTop: $(ref)[0].offsetTop
      }, 500, 'swing', function () {// Finished animation.
      });
    }
  });
}
...
function sidebarFunctionality() {
  ...
  var sectionInfo = []; // Store the current section that we're on!.

  var currentSection = ""; // Get the navigation title element.

  var navigationTitle = $('.navbar-header .nav-current-section'); // Get the sidebar elements.

  var sidebarElements = $('.main-sidebar ul > li'); // Loop through each section.
  ...
  for (var key in sections) {
    ...
    if (!isNaN(key) && sections.hasOwnProperty(key)) {
      var element = sections[key]; // Get information for each of the sections.

      var obj = {};
      obj.name = $(element).attr('section-name');
      obj.topPos = $(element).offset().top;
      obj.botPos = $(element).offset().top + $(element).outerHeight(true);
      obj.height = $(element).outerHeight(true); // Add this to the array.
...
function faqQuestionSetup() {
  $(document).on('click', '.faqs a', function () {
    ...
    $(".speech-bubble p").animate({
      opacity: 0
    }, function () {
      $(this).text(answer).animate({
        opacity: 1
      });
    }); // Move the eyes right
    // $('.eyes').css('left', '23px');
    ...
    $('.laptop-glow').show(); // Wait a second and return
